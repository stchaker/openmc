...
void write_message(int level, fmt::format_string<Params...> fmt_str, Params&&... fmt_args) {
  if (settings::verbosity >= level) {
    write_message(fmt::format(fmt_str, std::forward<Params>(fmt_args)...));
  }
}

template <typename... Params>
void write_message(fmt::format_string<Params...> fmt_str, Params&&... fmt_args) {
  write_message(fmt::format(fmt_str, std::forward<Params>(fmt_args)...));
}
