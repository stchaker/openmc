...
void print_results()
{
  ...
  if (n > 1) {
    ...
    std::tie(mean, stdev) = mean_stdev(&gt(GlobalTally::TR_COLLISION, 0), n);
    fmt::print(" Removal-Time (Collision) [mus] = {:.5f} +/- {:.5f}\n", 1e6 * mean,
      1e6 * t_n1 * stdev);
  } else {
    if (mpi::master)
    ...
    fmt::print(" Removal-Time (Collision) [mus] = {:.5f}\n",
      1e6 * gt(GlobalTally::TR_COLLISION, TallyResult::SUM) / n);
  }
  fmt::print("\n");
  std::fflush(stdout);
}
