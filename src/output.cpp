...
void print_results()
{
  ...
  if (n > 1) {
    ...
    if(settings::run_mode == RunMode::ALPHA){
      int avg_fission_counter = std::accumulate(simulation::fission_counter_tally.begin(), simulation::fission_counter_tally.end(), 0) / simulation::fission_counter_tally.size(); 
      int avg_alpha_counter = std::accumulate(simulation::alpha_counter_tally.begin(), simulation::alpha_counter_tally.end(), 0) / simulation::alpha_counter_tally.size(); 
