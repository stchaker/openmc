...
int openmc_simulation_finalize()
{
  ...
  if(!simulation::time_slice_bank_written && settings::num_neutrons_time_slice > 0){
    warning("Not enough time-slice neutron source sites were saved during the run!");
  }
...
void allocate_banks()
{
  ...
  if (settings::num_neutrons_time_slice > 0) {
    if(mpi::n_procs > 1) {
      simulation::time_slice_bank.reserve((settings::num_neutrons_time_slice / mpi::n_procs) * 2);
    } else{
...
void initialize_batch()
{
  ...
  if (first_inactive) {
    ...
    //Parallel reduce the max particle segment flight time if using OPENMPI
#ifdef OPENMC_MPI
    printf("reducing max flight times...\n");
    double reduced_max_flight_time;
    //Combine all values
    MPI_Allreduce(&simulation::max_track_segment_time, &reduced_max_flight_time, 1, MPI_DOUBLE, MPI_MAX, mpi::intracomm);
    printf("done!\n");
#endif 
...
void finalize_batch()
{
  ...
  if (settings::run_mode == RunMode::EIGENVALUE) {
    ...
    // Write the source sites generated by the time slice 
    if(settings::num_neutrons_time_slice > 0 && !simulation::time_slice_bank_written) {
      ...
      if(time_slice_population_parallel_scan.back() >= settings::num_neutrons_time_slice and settings::create_transient_source) {
        gsl::span<SourceSite> bankspan(simulation::time_slice_bank.begin(), simulation::time_slice_bank.size()); 
        write_source_point("transient_source", bankspan, time_slice_population_parallel_scan, false);
        simulation::time_slice_bank_written = true; 
      }
      else if(time_slice_population_parallel_scan.back() >= settings::num_neutrons_time_slice){
        gsl::span<SourceSite> bankspan(simulation::time_slice_bank.begin(), simulation::time_slice_bank.size()); 
