...
void calculate_alpha_ifp()
{
  ...
  for (auto i_tally = 0; i_tally < model::tallies.size(); i_tally++) {
    ...
    for (; filter_iter != end; ++filter_iter) {
      ...
      for (auto i_score = 0; i_score < model::tallies[i_tally]->scores_.size();
        i_score++) {
        // check for the correct tallies to be present and update the boolean
        // checks
        if (model::tallies[i_tally]->scores_[i_score] ==
            static_cast<int>(TallyScore::SCORE_IFP_TIME_NUM)) {
          ifp_num_time = true;
          std::tie(num_time, num_time_stdv) = mean_stdev(
            &tally.results_(filter_index, i_score, 0), tally.n_realizations_);
        } else if (model::tallies[i_tally]->scores_[i_score] ==
                   static_cast<int>(TallyScore::SCORE_IFP_BETA_NUM)) {
          ifp_num_beta = true;
          std::tie(num_beta, num_beta_stdv) = mean_stdev(
            &tally.results_(filter_index, i_score, 0), tally.n_realizations_);
        } else if (model::tallies[i_tally]->scores_[i_score] ==
                   static_cast<int>(TallyScore::SCORE_IFP_DENOM)) {
          ifp_denom = true;
          std::tie(denom, denom_stdv) = mean_stdev(
            &tally.results_(filter_index, i_score, 0), tally.n_realizations_);
        }
      }
    }
  ...
  if (!ifp_num_time || !ifp_num_beta || !ifp_denom) {
    fatal_error("IFP tallies are not present in the simulation, cannot "
                "calculate the alpha eigenvalue.");
  }

  // calculate kinetic parameters from tallies and from the simulation keff and
  // lambda_eff
  ...
  if (!settings::create_delayed_neutrons) {
    // if we are not using delayed neutrons, set the effective precursor decay
    // constant to 0
    lambda_eff = 0.0;
  }
  ...
  // Unsure about decay constant uncertainties, will assume this quantity has
  // none andt error comes from the approximation in the first place
  ...
  const double s_0_stdv =
    s_0 * (rho_stdv_rel + beta_stdv_rel + lambda_stdv_rel);
  const double s_1_stdv = s_1 * (rho_stdv_rel + beta_stdv_rel + mgt_stdv_rel);

  // determine the fundamental mode of the alpha eigenvalue by searching for the
  // most positive of the two root
  auto select_most_positive_nonzero = [](const double a,
    ...
    // Check if one of the values is zero, if the other is negative, return
    // anyway
    if (a == 0.0 && b < 0.0)
      return b;
    if (b == 0.0 && a < 0.0)
...
#pragma omp master
  {
    // Use inverse beta weighting to calculate the effective delayed neutron
    // precursor decay constant Uses a dummy value of 1 MeV to calculate the
    // value
    ...
    for (const auto& mat : model::materials) {
      // First we loop over all nuclides and store the atom density of only the
      // fissionable nuclides to the sum
    ...
    // Now we loop over all nuclides in materials and if fissionable, we
    // calculate the effective decay constant We then append the atom density of
    // the nuclide to the vector by dividing the atom density by the total
    // fissionable atom density
    for (const auto& mat : model::materials) {
      for (int i_nuclide : mat->nuclide_) {
        const auto& nuc = data::nuclides[i_nuclide];
        if (nuc->fissionable_) {
          ...
          for (int i = 1; i < (nuc->n_precursor_ + 1); i++) {
            // Calculate the effective decay constant for each delayed neutron
            // precursor this is done through inverse beta weighting for each
            // fissionable nuclide
            const auto& product = rx->products_[i];
            inverse_beta += (*product.yield_)(E_in) / product.decay_rate_;
            beta_eff += (*product.yield_)(E_in);
          }
          // append to the lambda_eff_nuclide vector and append the atom percent
          // of the nuclide to the nuclide_atom_percent vector
          lambda_eff_nuclide.emplace_back(beta_eff / inverse_beta);
          nuclide_atom_percent.emplace_back(
            mat->atom_density_[i_nuclide] / fissionable_atom_density);
        }
      }
    }

    // Calculate the effective decay constant for the entire simulation, signal
    // that this process executed successfully
...
void average_alpha()
{
  ...
  if (simulation::alpha_eigenvalue_tally.empty()) {
    ...
    simulation::alpha_eigenvalue_average =
      sum / simulation::alpha_eigenvalue_tally.size();
  }
}
    ...
    while (p.alive()) {
      ...
      if (p.alive()) {
        if ((p.collision_distance() > p.boundary().distance()) &&
            (p.alpha_event_distance() > p.boundary().distance())) {
